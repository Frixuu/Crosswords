...
class DictionaryJVM {
    ...
    companion object {
        var current: DictionaryJVM? = null
        ...
        suspend fun loadFromAsset(activity: Activity, filename: String): DictionaryJVM {

            val dict = DictionaryJVM()
            ...
            val parsingJobs = (0 until concurrentCount).map { i ->
                ...
                val endIndex = if (i == (startIndices.size - 1)) {
                    length.toInt()
                } else {
                    startIndices[i + 1]
                }
